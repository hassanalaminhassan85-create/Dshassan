...
export const CacTrustSection: React.FC<CacTrustSectionProps> = ({
  ...
  const handleShare = () => {
    ...
    const url = fileUrl || `${window.location.origin}/api/cac/file?key=${encodeURIComponent(activeCac.r2_object_key)}`;
  ...
  // Generate File URL dynamically with data URL / http fallback
  const getFileUrl = () => {
    if (!activeCac) return '';
    const key = activeCac.r2_object_key || '';
    if (!key || key === 'cac_certificate_default.png') {
      return ''; // Will render vector certificate
    }
    if (key.startsWith('http://') || key.startsWith('https://') || key.startsWith('data:')) {
      return key;
    }
    return `/api/cac/file?key=${encodeURIComponent(key)}`;
  };

  // Dynamic vector SVG of Nigerian CAC Certificate
  const renderVectorCacCertificate = (isInteractive: boolean = false) => {
    ...
    return (
      <div 
        style={scaleStyle}
        className="w-full max-w-[794px] aspect-[794/1123] bg-[#f2fcf6] p-4 sm:p-8 md:p-12 shadow-2xl relative select-none font-sans flex flex-col justify-between shrink-0 text-slate-800 border-[8px] sm:border-[16px] border-emerald-600 rounded-xl overflow-hidden text-[9px] sm:text-xs md:text-sm my-auto"
        id="cac-vector-cert"
      >
        ...
        <div className="px-10 text-center relative z-10 space-y-5 leading-relaxed text-slate-800 font-serif text-[13.5px] md:text-[14px]">
          ...
          <h2 className="text-red-600 font-sans font-black tracking-wide text-base md:text-[17.5px] border-b border-dashed border-emerald-600/30 pb-2 max-w-xl mx-auto uppercase">
            {activeCac?.company_name || 'DS TECH & DIGITAL MARKETING AGENCY LIMITED'}
          </h2>
        ...
        <div className="text-center relative z-10 font-serif text-[12.5px] text-slate-700 italic px-10 my-1">
          Given under my hand at <span className="font-bold font-sans not-italic text-slate-900">Abuja</span> this <span className="font-bold font-sans not-italic text-slate-900">
            {activeCac?.registration_date ? new Date(activeCac.registration_date).getDate() : '12'}th
          </span> day of <span className="font-bold font-sans not-italic text-slate-900">
            {activeCac?.registration_date ? new Date(activeCac.registration_date).toLocaleString('en-US', { month: 'long' }) : 'September'}, {activeCac?.registration_date ? new Date(activeCac.registration_date).getFullYear() : '2021'}
          </span>.
        </div>
  ...
  return (
    <section id="trust-compliance" className="relative py-16 md:py-24 px-4 sm:px-6 overflow-hidden bg-[#020B24] border-y border-blue-950">
      {/* Ambient Background Glows */}
      <div className="absolute top-1/4 left-1/4 w-[350px] h-[350px] bg-orange-600/10 rounded-full blur-[140px] pointer-events-none" />
      <div className="absolute bottom-1/4 right-1/4 w-[400px] h-[400px] bg-blue-600/10 rounded-full blur-[160px] pointer-events-none" />
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="flex items-center justify-between mb-8">
          <motion.button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-slate-900/90 hover:bg-slate-800 border border-slate-700/80 rounded-2xl text-slate-200 hover:text-white text-xs font-bold uppercase tracking-wider transition-all duration-200 cursor-pointer shadow-xl backdrop-blur-md"
          >
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-mono uppercase tracking-widest text-emerald-400 font-bold">REAL-TIME DB SYNCED</span>
          </div>
        </div>
        ...
        <div className="text-center max-w-2xl mx-auto mb-12 md:mb-16">
          <motion.div 
            ...
            viewport={{ once: true }}
            className="inline-flex items-center gap-2 px-3.5 py-1 bg-orange-500/10 border border-orange-500/25 rounded-full text-orange-400 text-xs font-bold uppercase tracking-wider mb-4 shadow-sm"
          >
            <ShieldCheck size={14} className="text-orange-400" />
            <span>Enterprise Regulatory Compliance</span>
          </motion.div>
          ...
          <motion.h2 
            ...
            className="text-3xl md:text-5xl font-black tracking-tight text-white uppercase font-serif"
          >
          ...
          <motion.p 
            ...
            className="text-xs md:text-sm text-slate-300 mt-4 leading-relaxed font-light"
          >
            Verify our official corporate credentials registered with the Corporate Affairs Commission of Nigeria. Our agency operates with strict legislative authorizations.
          </motion.p>
        </div>
        ...
        {loading ? (
          ...
          <div className="max-w-xl mx-auto bg-slate-900/40 border border-slate-800/80 rounded-3xl p-12 text-center space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-slate-400 text-sm">
                There are currently no active Corporate Affairs Commission certificates published.
              </p>
            </div>
            <button 
          ...
          <motion.div 
            ...
            className="w-full bg-gradient-to-b from-slate-900/80 to-slate-950/90 border border-slate-800/80 backdrop-blur-xl rounded-3xl p-6 md:p-10 shadow-2xl relative overflow-hidden text-left"
          >
            ...
            <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 justify-between items-stretch">
              ...
              <div className="flex-1 flex flex-col justify-between space-y-6">
                <div>
                  <div className="flex flex-wrap items-center gap-3 mb-4">
                    <span className="flex items-center gap-1.5 px-3 py-1 bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 text-[10px] font-black uppercase tracking-widest rounded-lg">
                  ...
                  <h3 className="text-xl md:text-3xl font-black text-white tracking-tight leading-tight uppercase font-serif">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-6 border-t border-slate-800/80 font-sans">
                  <div className="flex items-center gap-3">
                    <div className="p-2.5 bg-slate-800/60 rounded-xl border border-slate-700/40 text-orange-400 shrink-0">
                      <Building size={18} />
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-[10px] text-slate-400 uppercase font-bold tracking-wider block">INCORPORATED DATE</span>
                ...
                <div className="flex flex-wrap items-center gap-3 pt-4">
                  <motion.button
                    ...
                    className="px-5 py-3 bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-500 hover:to-amber-500 text-white text-xs font-black uppercase tracking-wider rounded-xl shadow-lg shadow-orange-600/20 transition-all duration-200 flex items-center gap-2 cursor-pointer"
                  >
                    <Eye size={16} />
                  ...
                  <motion.a
                    ...
                    className="px-4 py-3 bg-slate-800/90 hover:bg-slate-700/90 border border-slate-700 text-slate-300 hover:text-white text-xs font-bold uppercase tracking-wider rounded-xl transition-all duration-200 flex items-center gap-2"
                  >
              ...
              {/* Right Column: Mini Interactive Certificate Card Preview with Full Size Aspect Ratio */}
              <div className="w-full lg:w-[340px] flex items-center justify-center shrink-0">
                <motion.div 
                  whileHover={{ y: -6, scale: 1.02 }}
                  whileTap={{ scale: 0.98 }}
                  onClick={() => setIsViewerOpen(true)}
                  className="w-full aspect-[3/4] max-w-[300px] bg-slate-950 border-2 border-slate-800 rounded-2xl p-4 flex flex-col justify-between shadow-2xl relative overflow-hidden group cursor-pointer"
                >
                  <div className="absolute inset-0 bg-gradient-to-b from-transparent via-slate-950/20 to-slate-950/90 group-hover:to-slate-950/70 transition-colors pointer-events-none z-10" />
                  
                  {/* Certificate Preview Rendering */}
                  <div className="w-full flex-1 flex items-center justify-center relative overflow-hidden rounded-xl bg-slate-900/80 p-2 my-1 border border-slate-800/80">
                    {getFileUrl() ? (
                      activeCac.mime_type === 'application/pdf' ? (
                        <div className="w-full h-full flex flex-col items-center justify-center text-slate-300 p-4 text-center space-y-2">
                          <FileText size={42} className="text-orange-400 animate-bounce" />
                          <span className="text-xs font-bold uppercase tracking-wider text-slate-200">Uploaded PDF Certificate</span>
                          <span className="text-[10px] font-mono text-slate-400 line-clamp-1">{activeCac.file_name}</span>
                        </div>
                      ) : (
                        <img 
                          src={getFileUrl()} 
                          referrerPolicy="no-referrer"
                          alt={activeCac.company_name} 
                          className="w-full h-full object-contain rounded-lg group-hover:scale-105 transition-transform duration-300 shadow-md bg-white"
                        />
                      )
                    ) : (
                      <div className="w-full h-full flex items-center justify-center relative overflow-hidden scale-[0.45] origin-center">
                        {renderVectorCacCertificate(false)}
                      </div>
                    )}
                  </div>

                  <div className="relative z-20 text-center w-full space-y-1.5 mt-2">
                    <div className="mx-auto w-8 h-8 rounded-full bg-orange-500/20 border border-orange-500/40 flex items-center justify-center text-orange-400 shadow-md">
                      <Eye size={15} />
                    </div>
                    <span className="text-[11px] font-black uppercase tracking-wider text-orange-400 block">Click to Expand Document</span>
                    <span className="text-[9px] font-mono text-slate-400 block line-clamp-1">{formatCleanFileName(activeCac.file_name, activeCac.company_name)}</span>
                  </div>
                </motion.div>
              </div>
      ...
      {/* ==========================================
          ...
          ========================================== */}
      <AnimatePresence>
        {isViewerOpen && activeCac && (
          <motion.div 
            initial={{ opacity: 0, scale: 0.98 }}
            animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0, scale: 0.98 }}
            transition={{ duration: 0.25, ease: "easeOut" }}
            className="fixed inset-0 z-[100] bg-slate-950/98 backdrop-blur-2xl flex flex-col justify-between overflow-hidden text-slate-100"
            ref={viewerRef}
          >
            {/* 1. TOP HEADER CONTROLS */}
            <div className="p-3 md:p-4 bg-slate-900/95 border-b border-slate-800 flex flex-wrap items-center justify-between gap-3 text-white relative z-50 shadow-lg">
              <div className="flex items-center gap-3">
                <button
                  onClick={() => setIsViewerOpen(false)}
                  className="p-2 bg-slate-800 hover:bg-slate-700 text-slate-200 hover:text-white rounded-xl transition-all cursor-pointer border border-slate-700 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider"
                >
                  <ArrowLeft size={16} className="text-orange-400" />
                  <span className="hidden sm:inline">Back</span>
                </button>
                <div className="p-2 bg-orange-600/20 border border-orange-500/30 rounded-xl text-orange-400 hidden sm:block">
                  <ShieldCheck size={18} />
                </div>
              ...
              <div className="flex items-center gap-1 sm:gap-2 bg-slate-950/90 p-1 rounded-xl border border-slate-800 shadow-inner">
                <button 
                ...
                <button 
                  ...
                  className="px-2 py-1 text-[10px] font-extrabold font-mono bg-white/10 hover:bg-white/20 rounded text-slate-300 hover:text-white transition-colors cursor-pointer"
                >
                  RESET
                </button>
              ...
              <div className="flex items-center gap-2">
                <button 
                  onClick={handleShare}
                  className={`px-3 py-2 rounded-xl text-[10px] font-bold uppercase tracking-wider transition-all cursor-pointer flex items-center gap-1.5 shadow-sm ${
                    copySuccess ? 'bg-emerald-600 text-white' : 'bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white border border-slate-700'
                  }`}
                >
                  <Share2 size={13} />
                  <span className="hidden sm:inline">{copySuccess ? 'Link Copied!' : 'Share Document'}</span>
                </button>
                ...
                <button 
                  onClick={() => setIsViewerOpen(false)}
                  className="p-2 bg-orange-600 hover:bg-orange-500 text-white rounded-xl transition-all duration-150 cursor-pointer shadow-md"
                  title="Close (ESC)"
                >
                  <X size={16} />
                </button>
            ...
            {/* 2. VIEWER CORE PORT (SCROLLABLE & INTERACTIVE WITH ZERO OVERLAP) */}
            <div className="flex-1 overflow-auto flex items-center justify-center p-4 sm:p-8 bg-slate-950 relative scrollbar-thin">
              <div className="relative my-auto flex items-center justify-center">
                {getFileUrl() ? (
                  /* Custom uploaded image or PDF */
                  <div 
                    style={{ 
                      transform: `scale(${zoom}) rotate(${rotation}deg)`, 
                      transition: 'transform 0.2s cubic-bezier(0.16, 1, 0.3, 1)' 
                    }}
                    className="flex items-center justify-center max-w-full max-h-[82vh] shrink-0"
                  >
                    {activeCac.mime_type === 'application/pdf' ? (
                      <iframe 
                        src={getFileUrl()} 
                        className="w-[85vw] max-w-[850px] h-[78vh] bg-white border border-slate-700 rounded-2xl shadow-2xl" 
                      ...
                      <img 
                        src={getFileUrl()} 
                        referrerPolicy="no-referrer"
                        alt="CAC Official Certificate" 
                        className="max-w-[88vw] max-h-[78vh] w-auto h-auto object-contain rounded-2xl border-2 border-slate-800 shadow-2xl bg-white/95"
                      />
                    )}
                  </div>
                ) : (
            ...
            <div className="p-3 bg-slate-900/90 border-t border-slate-800 flex flex-wrap items-center justify-between gap-2 text-slate-400 text-[10px] font-mono relative z-50">
              <div className="flex items-center gap-4">
                <span>FILE: <span className="text-slate-100 font-bold">{formatCleanFileName(activeCac.file_name, activeCac.company_name)}</span></span>
                {activeCac.file_size > 0 && <span>SIZE: <span className="text-slate-100">{(activeCac.file_size / 1024).toFixed(1)} KB</span></span>}
                <span>TYPE: <span className="text-slate-100">{activeCac.mime_type || 'image/png'}</span></span>
              </div>
              <div className="flex items-center gap-1.5">
                <span className="px-2 py-0.5 bg-slate-800 rounded font-bold text-slate-300">Pinch / Scroll to zoom</span>
                <span className="px-2 py-0.5 bg-slate-800 rounded font-bold text-slate-300">Shortcuts: +/- | R</span>
              </div>
            </div>
